...
class CSVObject(object):
    ...
    def read(cls, file=None):
        ...
        if os.path.exists(file):
            cls.objects = []
            reader = csv.DictReader(open(file, 'r'), fieldnames=cls.fields, restkey="additional_data", restval=None,
                                    dialect='table')
            if cls.has_header:
                reader.next()
            for row in reader:
    ...
    def save(cls, file=None):
        file = cls.file if file is None else file
        w = csv.DictWriter(open(file, 'w'), fieldnames=cls.fields, restval="null", extrasaction='ignore',
                           dialect='table')
    ...
    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__, ",".join(map(lambda v: "%s=%s" % (v[0], v[1]),
...
def read_sets(string):
    ...
    """
    return frozenset(
        map(lambda cs: frozenset(map(lambda c: int(c) if c.isdigit() else str(c), cs.split(","))), string.split("|")))
...
def remove_results_with_warning(run_id, algorithms=None, dry_run=False):
    if algorithms is None:
        ...
        files = reduce(lambda l, f: l + [f[0]] + [f[1]],
                       map(lambda a: ['results-%s.txt' % a, 'output-%s.txt' % a], algorithms))
