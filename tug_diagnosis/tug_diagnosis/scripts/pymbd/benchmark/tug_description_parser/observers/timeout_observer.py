    ...
    def generate_model_parameter(config, topics_from_nodes):
        ...
        for topic in topics:
            ...
            for callerid in topics_from_nodes[topic]:
                ...
                rules.append(TimeoutObserver(ab_pred(str(callerid)), observation))

            new_vars, new_rules, new_nodes = CalleridsObserver.generate_model_parameter("timeout", topic, topics_from_nodes[topic])
...
class TestTimeoutObserver(unittest.TestCase):
    ...
    def test_timeout_observer(self):
        with self.assertRaises(ValueError):
            TimeoutObserver(ab_pred(""), "/topic")
        with self.assertRaises(ValueError):
            TimeoutObserver(ab_pred("/"), "/topic")
        with self.assertRaises(ValueError):
            TimeoutObserver("", "/topic")
        with self.assertRaises(ValueError):
            TimeoutObserver("/", "/topic")
        with self.assertRaises(ValueError):
            TimeoutObserver(ab_pred("name"), "")
        with self.assertRaises(ValueError):
            TimeoutObserver(ab_pred(""), "")
        with self.assertRaises(ValueError):
            TimeoutObserver("", "")
        with self.assertRaises(TypeError):
            TimeoutObserver(1, "/topic")
        with self.assertRaises(TypeError):
            TimeoutObserver(ab_pred("name"), 1)
        with self.assertRaises(ValueError):
            TimeoutObserver(ab_pred("name"), "/")
    ...
    def test_clause(self):
        ...
        self.assertEqual(str(new_clause.literals[1]), neg("/topic"), "Second literal in clause does not match!")

    def test_generate_model_parameter(self):
        config = {'topics': ['/topic'], 'type': 'timeout'}
        topics_from_nodes = {'/topic': ['/node1', '/node2']}
        vars, rules, nodes = TimeoutObserver.generate_model_parameter(config, topics_from_nodes)
        ...
        rules_req = [TimeoutObserver(ab_pred('/node1'), 'timeout_obs_/topic_/node1'),
                     TimeoutObserver(ab_pred('/node2'), 'timeout_obs_/topic_/node2'),
                     CalleridsObserver('timeout_obs_/topic_all', ['timeout_obs_/topic_/node1', 'timeout_obs_/topic_/node2'])]
        ...
        with self.assertRaises(KeyError):
            TimeoutObserver.generate_model_parameter({'topics_wrong_name': ['/topic'], 'type': 'timeout'}, topics_from_nodes)
        with self.assertRaises(KeyError):
            TimeoutObserver.generate_model_parameter({'type': 'timeout'}, topics_from_nodes)
        with self.assertRaises(KeyError):
            TimeoutObserver.generate_model_parameter({}, topics_from_nodes)
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter("not_a_dict", topics_from_nodes)
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter(1, topics_from_nodes)
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter({'topics': [], 'type': 'timeout'}, topics_from_nodes)
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter({'topics': [''], 'type': 'timeout'}, topics_from_nodes)
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter({'topics': [1], 'type': 'timeout'}, topics_from_nodes)
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter({'topics': "no_list", 'type': 'timeout'}, topics_from_nodes)
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter({'topics': 1, 'type': 'timeout'}, topics_from_nodes)
        ...
        config = {'topics': ['/topic'], 'type': 'timeout'}
        with self.assertRaises(KeyError):
            TimeoutObserver.generate_model_parameter(config, {'/topic_wrong_name': ['/node1', '/node2']})
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': []})
        with self.assertRaises(KeyError):
            TimeoutObserver.generate_model_parameter(config, {})
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter(config, "no_dict")
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter(config, 1)
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': ['/', '/node2']})
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': ['', '/node2']})
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': [1, '/node2']})
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': ['/node1', '/']})
        with self.assertRaises(ValueError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': ['/node1', '']})
        with self.assertRaises(TypeError):
            TimeoutObserver.generate_model_parameter(config, {'/topic': ['/node1', 1]})

    def test_decrypt_resource_info(self):
        self.assertEqual(TimeoutObserver.decrypt_resource_info("/topic_name [node1, node2]"), ['timeout_obs_/topic_name_node1', 'timeout_obs_/topic_name_node2'], "Topic name decryption not correct!")
        self.assertEqual(TimeoutObserver.decrypt_resource_info("/topic_name [node1, node2, node3, node4, node5]"), ['timeout_obs_/topic_name_node1', 'timeout_obs_/topic_name_node2', 'timeout_obs_/topic_name_node3',
                                                                                                               'timeout_obs_/topic_name_node4', 'timeout_obs_/topic_name_node5'], "Topic name decryption not correct!")
        self.assertEqual(TimeoutObserver.decrypt_resource_info("/topic_name [node1]"), ['timeout_obs_/topic_name_node1'], "Topic name decryption not correct!")
        self.assertEqual(TimeoutObserver.decrypt_resource_info("/topic_name {}"), ['timeout_obs_/topic_name_all'], "Topic name decryption not correct!")

        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/ [node1, node2]")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/ [node1, /]")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/topic_name")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/topic_name [/]")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/topic_name [/node1, ]")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/topic_name [/node1, /]")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("/")
        with self.assertRaises(ValueError):
            TimeoutObserver.decrypt_resource_info("")
        with self.assertRaises(TypeError):
            TimeoutObserver.decrypt_resource_info(1)
