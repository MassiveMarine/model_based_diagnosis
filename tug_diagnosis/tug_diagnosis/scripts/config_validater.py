...
class DiagnosisConfigValidater():
    ...
    def __init__(self, config, debug=False):
        ...
        self.obs_with_topics = ['hz', 'timestamp', 'timeout', 'timing', "score", 'velocity']
        ...
        for node in config.nodes:
            ...
            self.topics_from_nodes.update(node.pub_topic)
            self.topics_from_nodes.update(node.sub_topic)

        # read observation configuration
        self.topics = set()
        for obs in config.observers:
            if obs.type in self.obs_with_topics:
            # if 'topics' in obs:
                entry = obs.resource
...
if __name__ == "__main__":
    rospy.init_node('tug_diagnosis_config_validater', anonymous=False)
