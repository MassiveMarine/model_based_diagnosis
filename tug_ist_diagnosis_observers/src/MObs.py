...
class Multiple_Observer_Triggered(object):
    ...
    def start(self):
        ...
        if top_in_found == True:
            ...
            self.report_error()
        if top_in_found == True:
            msg_class_out = roslib.message.get_message_class(
                self.topic_type_out)
        else:
            self.report_error()
        rospy.Subscriber(self.topic_in, msg_class_in, self.callback_Triggering)
        rospy.Subscriber(
            self.topic_out, msg_class_out, self.callback_Triggered)
        rospy.spin()
        ...
        while True:
            ...
            if t2 == 0:
                ...
                self.pub.publish(Observations(time.time(),
                                 ['~Ok(' + self.param_out_topic[1:len(self.param_out_topic)] + '_Triggered)']))
